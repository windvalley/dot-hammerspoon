...
function _M.start()
	...
	-- ********** window position **********
	-- 居中
	bind(window_position.center.prefix, window_position.center.key, window_position.center.message, function()
		window_lib.moveAndResize("center")
	end)
	-- 左半屏
	bind(window_position.left.prefix, window_position.left.key, window_position.left.message, function()
		window_lib.moveAndResize("halfleft")
	end)
	-- 右半屏
	bind(window_position.right.prefix, window_position.right.key, window_position.right.message, function()
		window_lib.moveAndResize("halfright")
	end)
	-- 上半屏
	bind(window_position.up.prefix, window_position.up.key, window_position.up.message, function()
		window_lib.moveAndResize("halfup")
	end)
	-- 下半屏
	bind(window_position.down.prefix, window_position.down.key, window_position.down.message, function()
		window_lib.moveAndResize("halfdown")
	end)
	-- 左上角
	bind(window_position.top_left.prefix, window_position.top_left.key, window_position.top_left.message, function()
		window_lib.moveAndResize("cornerTopLeft")
	end)
	-- 右上角
	bind(window_position.top_right.prefix, window_position.top_right.key, window_position.top_right.message, function()
		window_lib.moveAndResize("cornerTopRight")
	end)
	-- 左下角
	bind(window_position.bottom_left.prefix, window_position.bottom_left.key, window_position.bottom_left.message, function()
		window_lib.moveAndResize("cornerBottomLeft")
	end)
	-- 右下角
	bind(window_position.bottom_right.prefix, window_position.bottom_right.key, window_position.bottom_right.message, function()
		window_lib.moveAndResize("cornerBottomRight")
	end)
	-- 左 1/3（横屏）或上 1/3（竖屏）
	bind(window_position.left_1_3.prefix, window_position.left_1_3.key, window_position.left_1_3.message, function()
		window_lib.moveAndResize("left_1_3")
	end)
	-- 右 1/3（横屏）或下 1/3（竖屏）
	bind(window_position.right_1_3.prefix, window_position.right_1_3.key, window_position.right_1_3.message, function()
		window_lib.moveAndResize("right_1_3")
	end)
	-- 左 2/3（横屏）或上 2/3（竖屏）
	bind(window_position.left_2_3.prefix, window_position.left_2_3.key, window_position.left_2_3.message, function()
		window_lib.moveAndResize("left_2_3")
	end)
	-- 右 2/3（横屏）或下 2/3（竖屏）
	bind(window_position.right_2_3.prefix, window_position.right_2_3.key, window_position.right_2_3.message, function()
		window_lib.moveAndResize("right_2_3")
	end)

	-- ********** window resize **********
	-- 最大化
	bind(window_resize.max.prefix, window_resize.max.key, window_resize.max.message, function()
		window_lib.moveAndResize("max")
	end)
	-- 等比例放大窗口
	bind(window_resize.stretch.prefix, window_resize.stretch.key, window_resize.stretch.message, function()
		window_lib.moveAndResize("stretch")
	end)
	-- 等比例缩小窗口
	bind(window_resize.shrink.prefix, window_resize.shrink.key, window_resize.shrink.message, function()
	...
	-- ********** window monitor **********
	-- 将窗口移动到上下左右或下一个显示器.
	bind(window_monitor.to_above_screen.prefix, window_monitor.to_above_screen.key, window_monitor.to_above_screen.message, function()
		log.d("move to above monitor")
		window_lib.moveToScreen("up")
	end)
	bind(window_monitor.to_below_screen.prefix, window_monitor.to_below_screen.key, window_monitor.to_below_screen.message, function()
		log.d("move to below monitor")
		window_lib.moveToScreen("down")
	end)
	bind(window_monitor.to_left_screen.prefix, window_monitor.to_left_screen.key, window_monitor.to_left_screen.message, function()
		log.d("move to left monitor")
		window_lib.moveToScreen("left")
	end)
	bind(window_monitor.to_right_screen.prefix, window_monitor.to_right_screen.key, window_monitor.to_right_screen.message, function()
		log.d("move to right monitor")
		window_lib.moveToScreen("right")
	end)
	bind(window_monitor.to_next_screen.prefix, window_monitor.to_next_screen.key, window_monitor.to_next_screen.message, function()
	...
	-- 关闭所有窗口
	bind(window_batch.close_all_windows.prefix, window_batch.close_all_windows.key, window_batch.close_all_windows.message, function()
